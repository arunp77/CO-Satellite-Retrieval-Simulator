...
MC_DATA_DIR = os.path.join(SCRIPT_DIR, "mc_data")
...
class MCResult:
    ...
    def save(self, path: str) -> None:
        ...
        Example
        -------
        >>> result.save("mc_data/noise_ensemble")
        # writes  mc_data/noise_ensemble.npz
        """
        ...
        np.savez_compressed(
            ...
            xco2_true      = self.xco2_true,
            xco2_retrieved = self.xco2_retrieved,
            xco2_unc       = self.xco2_unc,
            albedo_true    = self.albedo_true,
            sza_true       = self.sza_true,
            snr_true       = self.snr_true,
            n_iter         = self.n_iter,
            converged      = self.converged,
        )
        saved = path if path.endswith(".npz") else path + ".npz"
        size  = os.path.getsize(saved)
        label = f"{size/1024:.1f} KB" if size < 1_048_576 else f"{size/1_048_576:.2f} MB"
        print(f"  Saved  {self.n_samples} samples  ->  {saved}  ({label})")

    @classmethod
    def load(cls, path: str) -> "MCResult":
        """Load an MCResult previously saved with :meth:`save`.

        Example
        -------
        >>> result = MCResult.load("mc_data/noise_ensemble")
        """
        ...
        obj = cls(
            ...
            xco2_true      = data["xco2_true"],
            xco2_retrieved = data["xco2_retrieved"],
            xco2_unc       = data["xco2_unc"],
            albedo_true    = data["albedo_true"],
            sza_true       = data["sza_true"],
            snr_true       = data["snr_true"],
            n_iter         = data["n_iter"],
            converged      = data["converged"],
        )
        print(f"  Loaded {obj.n_samples} samples  <-  {path}")
        return obj

    def to_dataframe(self):
        """Return all arrays as a pandas DataFrame.

        Columns: mode, xco2_true, xco2_retrieved, xco2_unc,
                 albedo_true, sza_true, snr_true, n_iter, converged, error
        """
        try:
            import pandas as pd
...
def load_or_run(path: str, run_func, force: bool = False) -> MCResult:
    """Load a cached MCResult, or run the simulation if no cache exists.

    On first call: runs the (slow) simulation, saves to disk, returns result.
    On every subsequent call: loads instantly, skips the simulation entirely.

    Parameters
    ----------
    path : str
        Path for the .npz cache file (extension optional).
    run_func : callable
        Zero-argument callable that returns an MCResult.
        Called only when no cache exists or force=True.
    force : bool
        If True, always re-run even if a cache exists.

    Example
    -------
    >>> res = load_or_run(
    ...     "mc_data/noise_ensemble",
    ...     lambda: run_noise_ensemble(nu, xsec_func, N_col_func, n_samples=300),
    ... )
    """
    npz_path = path if path.endswith(".npz") else path + ".npz"
    if not force and os.path.isfile(npz_path):
        print(f"  Cache hit  ->  loading '{npz_path}' (skipping simulation)")
        return MCResult.load(npz_path)
    print(f"  Cache miss ->  running simulation and saving to '{npz_path}'")
    result = run_func()
    result.save(path)
    return result
...
def parse_args():
    ...
    p.add_argument("--force",    action="store_true",      help="Re-run all simulations even if cache exists")
    return p.parse_args()
...
def main():
    ...
    # ------------------------------------------------------------------
    # Run all five experiments — cached after first run
    # ------------------------------------------------------------------
    print(f"\n  Data cache directory: {MC_DATA_DIR}/")
    print(f"  (Simulations are skipped automatically if cache files exist)")

    res_noise = load_or_run(
        os.path.join(MC_DATA_DIR, "noise_ensemble"),
        lambda: run_noise_ensemble(
            nu, xsec_func, N_col_func,
    ...
    res_sweep = load_or_run(
        os.path.join(MC_DATA_DIR, "xco2_sweep"),
        lambda: run_xco2_sweep(
            nu, xsec_func, N_col_func,
    ...
    res_random = load_or_run(
        os.path.join(MC_DATA_DIR, "random_scenes"),
        lambda: run_random_scenes(
            nu, xsec_func, N_col_func,
    ...
    # ------------------------------------------------------------------
    # Sensitivity studies (one cache file per SNR / SZA level)
    # ------------------------------------------------------------------
    snr_values = [75, 100, 150, 200, 300, 400]
    snr_results = {}
    for snr in snr_values:
        snr_results[snr] = load_or_run(
            os.path.join(MC_DATA_DIR, f"snr_sens_{int(snr):04d}"),
            lambda s=snr: run_noise_ensemble(
                nu, xsec_func, N_col_func,
                n_samples=args.n_snr,
                xco2_true_ppm=435.0, xco2_prior_ppm=420.0,
                snr=s, sza=30.0, seed=args.seed,
            ),
            force=args.force,
        )
    ...
    sza_results = {}
    for sza in sza_values:
        sza_results[sza] = load_or_run(
            os.path.join(MC_DATA_DIR, f"sza_sens_{int(sza):03d}"),
            lambda z=sza: run_noise_ensemble(
                nu, xsec_func, N_col_func,
                n_samples=args.n_sza,
                xco2_true_ppm=435.0, xco2_prior_ppm=420.0,
                snr=250.0, sza=z, seed=args.seed,
            ),
            force=args.force,
        )
